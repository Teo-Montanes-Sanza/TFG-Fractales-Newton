...
def espacio_parametros_newton_granular(resolucion=1500, max_iter=60, tolerancia=1e-4):
    a_min, a_max = -2.5, 2.5
    a_im_min, a_im_max = -2.5, 2.5
...
plt.xticks(np.arange(-2.5, 3.0, 0.5), fontsize=10, family='serif')
plt.yticks(np.arange(-2.5, 3.0, 0.5), fontsize=10, family='serif')

plt.xlabel(r"$Re(a)$", fontsize=14, family='serif')
plt.ylabel(r"$Im(a)$", fontsize=14, family='serif')

plt.savefig("Figura4.4.png", bbox_inches='tight')
